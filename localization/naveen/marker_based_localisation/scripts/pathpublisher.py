...
def callback(data):
    ...
    #To avoid repeating the values, it is found that the received values are differents
        if (xAnt != pose.pose.position.x and yAnt != pose.pose.position.y and zAnt != pose.pose.position.z):
            #Set a atributes of the msg
            pose.header.seq = path.header.seq + 1
            path.header.frame_id="map_fid"
...
def posecallback(data):
    ...
    #To avoid repeating the values, it is found that the received values are differents
        if (xAnt != data.pose.position.x and yAnt != data.pose.position.y and zAnt != data.pose.position.z):
            #Set a atributes of the msg
            data.header.seq = path.header.seq + 1
            path.header.frame_id="map_fid"
            path.header.stamp=rospy.Time.now()
            data.header.stamp = path.header.stamp
    ...
    #Save the last position
        xAnt=data.pose.orientation.x
        yAnt=data.pose.position.y
        zAnt=data.pose.position.z
        return path
...
if __name__ == '__main__':
    ...
    msg = rospy.Subscriber('/ground_truth/pose', PoseStamped, posecallback) 
